  ...
  <div class="concentrator">
    ...
    <div class="contant">
      <el-table id="table" :data="tableData" border height="530" style="width: 100%">
        ...
        <el-table-column align="center" :show-overflow-tooltip="true" prop="name" label="设备名称">
          <template slot-scope="scope">{{scope.row.name === null?'空':scope.row.name}}</template>
        </el-table-column>
        <el-table-column align="center" :show-overflow-tooltip="true" prop="code" label="设备编号">
        ...
        >
          <template
            slot-scope="scope"
          >{{scope.row.sim === null?'空':scope.row.sim}}</template>
        </el-table-column>
        <el-table-column
          align="center"
          :show-overflow-tooltip="true"
        ...
        >
          <template
            slot-scope="scope"
          >{{scope.row.imsi === null?'空':scope.row.imsi}}</template>
        </el-table-column>
        <el-table-column
          align="center"
          :show-overflow-tooltip="true"
        ...
        >
          <template
            slot-scope="scope"
          >{{scope.row.imei === null?'空':scope.row.imei}}</template>
        </el-table-column>
        <el-table-column
          align="center"
          :show-overflow-tooltip="true"
        ...
        >
          <template
            slot-scope="scope"
          >
            <div v-show="scope.row.dtype == null">空</div>
            <div v-show="scope.row.dtype == 21">烟感探测器</div>
            <div v-show="scope.row.dtype == 110">电气火灾监控设备</div>
            <div v-show="scope.row.dtype == 101">可燃气体探测器</div>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="NewSys" width="30%">
      <el-form ref="form" :model="form" size="small" label-width="100px">
        ...
        <el-form-item required  label="设备类型">
          <el-col :span="13">
            <el-input maxlength="14" v-model="form.dtype"></el-input>
        ...
        <el-form-item required label="SIM编号">
          <el-col :span="13">
            <el-input v-model="form.sim"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item required label="IMSI">
          <el-col :span="13">
            <el-input v-model="form.imsi"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item required label="IMEI">
          <el-col :span="13">
...
export default {
  data() {
    return {
      url:'http://193.168.1.114:11030/bus/equipment/importExcel',
      fileList:[],
      forms:{
        file:''
      },
  ...
  methods: {
    ...
    change(row) {
      this.form = row;
      this.title = "编辑" + row.name + "设备";
      this.update = true;
    },
    sureChange() {
      let data = this.form;
      getupdateEquipment(data).then(res => {
        if (res.data.code == 1) {
          this.$message({
            message: "修改成功",
            type: "success"
    ...
    deleteRow(row) {
      this.$confirm(
        ...
        .then(() => {
          ...
          getdeleteEquipment(data).then(res => {
            if (res.data.code == 1) {
              this.$message.success("成功删除了" + row.name + "设备!");
              this.getList();
            } else {
              this.$message.error(res.data.msg);
            }
    ...
    // 导出excel文档
    clickExcel() {
      getEquipmentExcel().then(res=>{
        console.log('res--',res)
        const content = res.data
        const blob = new Blob([content]) // 构造一个blob对象来处理数据
        const fileName = 'equipment.xlsx' // 导出文件名
