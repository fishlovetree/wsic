  ...
  <div class="concentrator">
    ...
    <div class="contant">
      <el-table id="table" :data="tableData" border height="530" style="width: 100%">
        ...
        <el-table-column align="center" :show-overflow-tooltip="true" prop="name" label="水表名称">
          <template slot-scope="scope">{{scope.row.name === null?'空':scope.row.name}}</template>
        </el-table-column>
        <el-table-column align="center" :show-overflow-tooltip="true" prop="code" label="水表编号">
    ...
    <el-dialog :title="title" :visible.sync="NewSys" width="30%">
      <el-form ref="form" :model="form" size="small" label-width="100px">
        <el-form-item required label="水表名称">
          <el-col :span="13">
            <el-input v-model="form.name"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item required  label="水表编号">
          <el-col :span="13">
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          getdeleteMeter(data).then(res => {
            if (res.data.code == 1) {
              this.$message.success("成功删除了" + row.name + "水表!");
              this.getList();
            } else {
              this.$message.error(res.data.msg);
            }
